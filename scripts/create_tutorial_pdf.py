...
def cover_page(canvas, doc) -> None:
    ...
    canvas.drawString(23 * mm, PAGE_H - 151 * mm, "Windows 软件 · 赛季隔离 · 离线识别 · AA 分摊 · 无损更新")
...
def make_document() -> None:
    ...
    doc = BaseDocTemplate(
        ...
        subject="经费管理系统赛季隔离、批量台账、离线识别、AA、无损更新与回溯说明",
    )
    ...
    story += [
        ...
        grid(
            ...
            [
                ...
                ["创作者", "查看全赛季共享的姓名、组别与岗位", "全员查看 / 管理员编辑"],
            ],
            [34, 87, 45],
        ),
        PageBreak(),
        ...
        p("支持 JPG、PNG、BMP、TIFF、WEBP、PDF、TXT、OFD、ZIP 和多文件直接导入。文字识别、费用分类和物品类型判断均在本机完成；发票图片与数据库保存在 Windows 固定数据区。"),
        process([
            ("上传", "选择图片或 PDF"),
            ("识别", "点击离线 OCR"),
            ("核对", "校验号码、日期、金额"),
        ...
        *bullets([
        ...
        p("赛季、长期组别与创作者", h2),
        *bullets([
            "只有管理员可新建、切换、改名、归档或重新启用赛季；普通成员和公共账号不能切换。",
            "首次升级至 V2.2.1 时，已有业务记录与成员账号自动归入 2026 赛季，旧数据不会删除。",
            "新赛季不沿用原成员和成员账号，需要管理员重新建立；管理员和公共查看账号跨赛季保留。",
            "电气部、底盘部、车身部、市场部等长期组别跨赛季保留，归档赛季自动只读。",
            "创作者名单在所有历史和新建赛季中共享，所有人可查看、只有管理员可编辑；默认记录为电气部 / 高压 / 刘松宁。",
        ]),
        p("分类与资金来源", h2),
        *bullets([
        ...
        p("多选、追溯与导出", h2),
        p("台账可按关键词、分类、报销状态和日期检索，显示开票及上传日期、提交人和账号。多选后可批量改分类、报销状态、删除或导出。CSV 可直接用 Excel 打开，完成后提示数量与金额。"),
        PageBreak(),
        ...
        *bullets([
        ...
        grid(
            ["位置", "内容", "迁移要求"],
            [
                ["固定数据区 data/", "账号、人员、组别、发票、分摊、结算、日志和版本", "由完整备份保留"],
                ["固定数据区 uploads/", "发票、票据、背景和登录轮播媒体", "由完整备份保留"],
                ["固定数据区 models/", "PP-OCRv5 离线模型", "更新不会覆盖"],
                ["固定数据区 data/backups/", "自动更新前与恢复操作前的保护备份", "软件自动管理"],
            ],
            [55, 67, 44],
        ),
        p("完整备份", h2),
        *bullets([
            "管理员进入“系统设置”，点击下载完整备份，并选择保存位置。",
            "备份包包含数据库、附件，以及人员、组别、账号关联和发票明细可查看清单。",
            "每周至少一次；赛前、集中采购后、批量回溯前额外备份。",
            "恢复备份会替换当前业务数据，请先再导出一份现状备份。",
        ]),
        callout("迁移电脑与演示数据", "推荐先下载完整备份，再在新电脑安装软件并恢复备份，不要只复制 EXE。首次启动含 5 条演示发票和 8 位示例成员；熟悉功能后可在系统设置中一键清除演示发票，成员与账号不会一起删除。", RED),
        PageBreak(),
        ...
        *bullets([
            ...
            "接收云端变更前建立同步保护点。",
            "恢复历史版本前建立“回溯前自动保护点”。",
            "每个赛季最多保留最近 100 个版本，旧版本按时间自动淘汰。",
        ]),
        p("回溯范围", h2),
        grid(
            ["会恢复", "不会恢复", "回溯后动作"],
            [
                ["当前赛季的成员、附件记录、发票、分摊和结算", "其他赛季、创作者名单、登录密码、长期组别、全局设置和会话", "生成当前赛季同步事件，其他赛季保持不变"],
            ],
            [62, 52, 52],
        ),
        Spacer(1, 5 * mm),
        ...
        *bullets([
        ...
        p("无损更新", h2),
        *bullets([
            "V2.2.4 起所有账号均可进入“系统设置 → 版本与更新”，选择自动完整备份并更新，或手动下载轻量补丁覆盖。",
            "自动方式会先备份全部赛季、人员、组别、账号、发票和附件；校验 SHA-256 后再替换程序，固定数据区不会被更新包覆盖。",
            "从 V2.2.0 更新至 V2.2.1 会自动把既有数据归入 2026 赛季，不需要手工迁移。",
        ...
        *bullets([
            ...
            "当前赛季名称正确；新赛季成员账号已重新建立，长期组别与创作者名单已核对。",
            "分类和资金来源符合本赛季财务口径。",
